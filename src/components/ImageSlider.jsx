...
function ImageSlider({ images }) {
  ...
  const slider = images.map((img, index) => (
    <React.Fragment key={index + img}>
      <figure
        ...
        className='ease-in-out h-full min-w-full rounded-lg bg-cover bg-center duration-500'
      ></figure>
    </React.Fragment>
  ));
  ...
  return (
    <div className="relative flex h-full w-full">
      {slider}
      <button
        className="group absolute left-0 top-0 h-full w-[12%] duration-300 hover:bg-semiDark rounded-l-lg"
        onClick={previousSlide}
      >
        <span className="arrow left float-left ml-[40%] duration-300 group-hover:ml-[30%]"></span>
      </button>
      <button
        className="group absolute right-0 top-0 h-full w-[12%] duration-300 hover:bg-semiDark rounded-r-lg"
        onClick={nextSlide}
      >
